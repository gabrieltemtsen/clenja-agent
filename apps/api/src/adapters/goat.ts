...
class GoatTurnkeyEvmWalletClient extends EVMWalletClient {
  constructor(private readonly address: `0x${string}`, private readonly walletId: string, private readonly userId: string) {
  ...
  async sendTransaction(tx: EVMTransaction): Promise<{ hash: string }> {
    ...
    const unsignedTx = ethers.utils.serializeTransaction({
      ...
      gasLimit: tx.value ? 21000 : 300000,
      to: tx.to,
    ...
    store.addAudit({ id: `aud_${Date.now()}`, ts: Date.now(), userId: this.userId, action: "goat.turnkey.sendTransaction", status: "ok", detail: { txHash, walletId: this.walletId } });
